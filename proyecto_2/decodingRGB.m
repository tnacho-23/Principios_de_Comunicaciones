...
%Se mide cuando se activa la señal de sincronizacion. Para hacer esto se toman
%segmentos de la señal de 2048 muestras y se analiza su espectro de frecuencias
%segmento
...
s_sincr=[s_sincr1, s_sincr2, s_sincr3]; % se concatenan tres tonos

%acorr contiene la correlacion para cada desplazamiento lag
header_test=header_time*30;

signal=s(1: header_test*fs); % se busca en una ventana del doble de la señal de sincronizacion
...
%------------------------------demodulacion------------------------------
for i=1:400
y= s(m_pixel*(i-1)+lagDiff:m_pixel*i+lagDiff); % se filtra por cantidad de muestras para cada pixel
